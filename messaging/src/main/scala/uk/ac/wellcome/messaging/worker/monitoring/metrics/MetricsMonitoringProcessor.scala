...
final class MetricsMonitoringProcessor[Message, ProcessMonitoringClient <: MetricsMonitoringClient](val namespace: String)(implicit val monitoringClient: ProcessMonitoringClient)
  extends MonitoringProcessor[Message, Instant] with Logger with MetricsProcessor {

  override def recordStart(message: Message)(implicit ec: ExecutionContext): Future[Instant] = Future.successful(Instant.now)
