...
trait IndividualMessageSender[Destination, MessageAttributes] {
  def send(body: String, attributes: Option[MessageAttributes])(subject: String, destination: Destination): Try[Unit]

  def sendT[T](t: T, attributes: Option[MessageAttributes])(subject: String, destination: Destination)(
...
trait MessageSender[MessageAttributes] {
  ...
  protected val underlying: IndividualMessageSender[Destination, MessageAttributes]
  ...
  def sendT[T](t: T, attributes: Option[MessageAttributes])(implicit encoder: Encoder[T]): Try[Unit] =
