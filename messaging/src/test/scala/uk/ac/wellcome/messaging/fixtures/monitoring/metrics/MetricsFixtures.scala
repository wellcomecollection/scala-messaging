...
trait MetricsFixtures extends Matchers {
  ...
  def withMetricsMonitoringProcessor[Work, R](namespace: String, shouldFail: Boolean = false)(testWith: TestWith[(FakeMetricsMonitoringClient, MetricsMonitoringProcessor[Work]), R])(implicit ec: ExecutionContext): R = {
    withFakeMonitoringClient(shouldFail) { client: FakeMetricsMonitoringClient =>
      val metricsProcessor = new MetricsMonitoringProcessor[Work](namespace)(client, ec)
